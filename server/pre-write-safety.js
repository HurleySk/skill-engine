...
const DEFAULT_SAFETY_RULES = {
  ...
  prodEnvironments: ['acme', 'orgprod', 'PRODORG', 'prodorg4', 'prodorg5', 'prodorg6'],
  ...
  prodUriPatterns: [
    'acme.crm.dynamics.com', 'prodorg2.crm.dynamics.com', 'prodorg3.crm.dynamics.com',
    'prodorg4.crm.dynamics.com', 'prodorg5.crm.dynamics.com', 'prodorg6.crm.dynamics.com',
    'sqlprod.example.net', 'ONPREM-SQLPROD'
  ],
  ...
  prodUriRegex: 'acme\\.crm|prodorg4\\.crm|prodorg5\\.crm|prodorg6\\.crm|prodorg3\\.crm|prodorg2\\.crm',
...
function validateProdOrgConfig(content, rules) {
  ...
  while ((m = tupleRe.exec(content)) !== null) {
    ...
    if (/dev\.crm|devorg|devorg2|devorg3/i.test(svcUri)) {
      if (envName === 'prod') {
...
function handlePreWrite(input, projectRoot) {
  ...
  if (/^tasks\/.*\.json$/.test(relPath)) {
    check = 'task';
  } else if (/work-repo-staging\/.*ProdOrgConfig/.test(relPath)) {
    check = 'secmodel';
  }
  ...
  if (check === 'secmodel') {
    const result = validateProdOrgConfig(content, rules);
    if (result) {
      return preWriteResponse(result.decision, result.reason);
    }
  }
...
module.exports = { handlePreWrite, DEFAULT_SAFETY_RULES, loadSafetyRules, validateTaskSteps, validateProdOrgConfig };
