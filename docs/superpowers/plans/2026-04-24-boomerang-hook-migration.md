...
case "$REL_PATH" in
  ...
  work-repo-staging/*ProdOrgConfig*) CHECK="workrepo-staging" ;;
  *) exit 0 ;;
esac
...
if [[ "$CHECK" == "task" ]]; then
  ...
  # Fallback defaults
  : "${PROD_FACTORIES:=prd}"
  : "${PROD_CONNECTIONS:=prd|onpremprod}"
  : "${PROD_ENVIRONMENTS:=acme|orgprod|PRODORG|prodorg4|prodorg5|prodorg6}"
  ...
  : "${PROD_URI_PATTERNS:=acme.crm.dynamics.com|prodorg2.crm.dynamics.com|prodorg3.crm.dynamics.com|prodorg4.crm.dynamics.com|prodorg5.crm.dynamics.com|prodorg6.crm.dynamics.com|sqlprod.example.net|ONPREM-SQLPROD}"
  ...
  : "${PROD_URI_RE:=acme\.crm|prodorg4\.crm|prodorg5\.crm|prodorg6\.crm|prodorg3\.crm|prodorg2\.crm}"
  : "${PROD_NAME_RE:=\bprod\b|PRODORG|orgprod}"
...
if [[ "$CHECK" == "workrepo-staging" ]]; then
  ...
  while IFS=$'\t' read -r ENV_NAME DV_NAME SVC_URI; do
    ...
    if echo "$SVC_URI" | grep -qEi "dev\.crm|devorg|devorg2|devorg3"; then
...
- `work-repo-staging/*` case now matches `*ProdOrgConfig*` directly in the case pattern instead of checking inside the if block
