...
3. **Enforcement**: Default to `warn` unless the user explicitly says "block" or "prevent." Use `ask` (or `approve` — treated identically) when the user wants approval-before-proceeding (prompts the user, they can approve or deny). PostToolUse and Stop rules cannot block or ask — use `suggest` or `warn`.
